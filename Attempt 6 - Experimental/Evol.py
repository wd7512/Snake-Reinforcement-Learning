...
class Layer_Dense: #hidden layer
    def __init__(self, n_inputs, n_neurons, fan_tot):
        self.w_bounds = np.sqrt(6/fan_tot)
        self.weights = np.random.uniform(-self.w_bounds,self.w_bounds,size=(n_inputs, n_neurons))
...
def mask(array,prob):
    mask = np.random.randint(0,2,size = array.shape)

    for i in range(prob-1):
        mask = mask * np.random.randint(0,2,size = array.shape)
...
class Brain:
    def __init__(self):
        self.lay1 = Layer_Dense(24,8,24+24*8)
        self.act1 = Activation_ReLU()
        self.lay2 = Layer_Dense(8,4,24*8+8*4)
        self.act2 = Activation_ReLU()
        self.fin = Activation_Softmax()
    def forward(self, inputs):
        ...
        self.fin.forward(self.act2.output)

        self.output = self.fin.output
...
def multi_mutate(brains, x): #keeps the brains and makes x many children asexually
    ...
    for i in range(len(brains)):
        ...
        for i in range(x):
            ...
            new_brain.lay1.weights += mask(new_brain.lay1.weights,5) * np.random.normal(loc=0,scale=new_brain.lay1.w_bounds,size=(24,8))
            new_brain.lay1.biases += mask(new_brain.lay1.biases,5) * np.random.normal(loc=0,scale=new_brain.lay1.w_bounds,size=(1,8))
            new_brain.lay2.weights += mask(new_brain.lay2.weights,5) * np.random.normal(loc=0,scale=new_brain.lay2.w_bounds,size=(8,4))
            new_brain.lay2.biases += mask(new_brain.lay2.biases,5) * np.random.normal(loc=0,scale=new_brain.lay2.w_bounds,size=(1,4))
...
def learn(networks,gens):
    ...
    for i in range(gens):
        ...
        for brain in trial_brains: #THIS IS WHERE YOUR SCORING / LOSS FUNCTION GOES
            score = multi_test(brain,2) #score each brain
            scores.append(score)
